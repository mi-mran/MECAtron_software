#this implementation is to detect ALL defined colors, regardless of whether the bounding boxes overlap or not
...
while True:
	...
	#note that this implementation will give multiple, overlapping bounding boxes which may be hard to visualize in real time
	def get_bbox_all(color, mask):
		#return values are image, contours, hierarchy. we are interested in the contours of each colored object
		_, contours, _ = cv2.findContours(mask.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

		#iterate through each contour of the selected color
		for contour in contours:
			area = cv2.contourArea(contour)

			#if object is less than 500px, do not recognise it in a bounding box
			if area > 500:
				...
				cv2.putText(cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2), color, (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0))

	get_bbox_all('red', red_mask)
	get_bbox_all('blue', blue_mask)
	get_bbox_all('green', green_mask)
	#get_bbox_all('orange', orange_mask)

	#displays the video output
	cv2.imshow('MECAtron Task 1 - Color Detection (All)', frame)
